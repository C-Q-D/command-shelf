...
重要约束：构建缓存、证据和运行期临时文件默认保存在项目 `.local`；脚本不会安装应用、修改用户配置、访问远端仓库或清理调用方已有目录。
...
param(
    [Parameter()]
    [string]$TargetDirectory = ".local\release-target",

    [Parameter()]
    [string]$EvidenceDirectory = ".local\release-evidence"
)
...
$resolvedRuntimeTempDirectory = Resolve-OutputDirectory -Path ".local\runtime-temp" -RepositoryRoot $repositoryRoot
...
try {
    ...
    # Cargo 缓存与测试期临时文件都固定在项目 `.local`，避免再次占用系统盘或真实用户配置。
...
'@
    ...
    $evidence = [ordered]@{
        ...
        runtimeTempDirectory = $resolvedRuntimeTempDirectory
        gitCommit = $gitCommit
    ...
    if ($null -eq $previousTempDirectory) {
        Remove-Item Env:TEMP -ErrorAction SilentlyContinue
    }
    else {
        $env:TEMP = $previousTempDirectory
    }
    if ($null -eq $previousTmpDirectory) {
